    ...
    export default Vue.extend({
        ...
        methods: {
            ...
            searchForm(formName: string) {
                let t = this;
                Vue.axios.post("/room/list", t.$data[formName]).then(function (res) {
            ...
            toDelete(index: number, row: any) {
                let t = this;
                Vue.axios.post("/room/deleteById/" + row.id).then(function (res) {
                    let data = res.data;
                    if (data.code == 200) {
                        t.$message({
                            message: '保存成功!',
            ...
            showDialogForm(id: Number) {
                let t = this;
                Vue.axios.post("/room/findById/" + id).then(function (res) {
                    let data = res.data;
                    if (data.code == 200) {
                        t.dialog.form = {
                            id: data.entity.id,
