  ...
  <div id="app">
      ...
      <el-form :model="queryForm" ref="queryForm" label-width="110px">
        ...
        <el-col :span="6">
          <el-form-item label="学号">
            <el-input v-model="queryForm.schoolNo" clearable></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="班级">
            <el-select v-model="queryForm['schoolClass.id']" filterable clearable>
              <el-option
        ...
        <el-col :span="6">
          <el-form-item label="学年">
            <el-select v-model="queryForm['schoolYear.id']" filterable clearable>
              <el-option
    ...
    <el-row>
      <el-table :data="tableData" border style="width: 100%" stripe>
        ...
        <el-table-column prop="schoolNo" label="学号"/>
        <el-table-column prop="idCard" label="身份证号"/>
        <el-table-column prop="schoolClass.name" label="班级"/>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <el-button
              @click.prevent="toDelete(scope.$index, scope.row)"
              type="text"
              size="small"
            >删除</el-button>
          </template>
    ...
    <el-dialog :visible.sync="dialog.show">
      ...
      >
        ...
        <el-form-item label="学号" prop="schoolNo">
          <el-input v-model="dialog.form.schoolNo" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="身份证号" prop="idCard">
          <el-input v-model="dialog.form.idCard" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="班级" prop="schoolClass">
          <el-select v-model="dialog.form.schoolClass" filterable clearable>
            <el-option
              v-for="item in classOptions"
              :key="item.id"
              :label="item.name"
...
export default Vue.extend({
  data() {
    return {
      ...
      dialog: {
        ...
        rules: {
          name: [{ required: true, message: "请输入姓名名", trigger: "blur" }],
          schoolNo: [
  ...
  methods: {
    loadYearOptions() {
      ...
      Vue.axios.post("year/all").then(res => {
        if (res.data.code == 200) {
          t.yearOptions = res.data.entity;
        }
      });
    },
    loadClassOptions() {
      let t = this;
      Vue.axios.post("class/all").then(res => {
        if (res.data.code == 200) {
          t.classOptions = res.data.entity;
        }
      });
    },
    ...
    toDelete(index: number, row: any) {
      let t = this;
      Vue.axios.post("/student/deleteById/" + row.id).then(function(res) {
        let data = res.data;
        if (data.code == 200) {
          t.$message({
            message: "保存成功!",
    ...
    showDialogForm(id: Number) {
      let t = this;
      Vue.axios.post("/student/findById/" + id).then(function(res) {
        let data = res.data;
        if (data.code == 200) {
          t.dialog.form = {
            id: data.entity.id,
            name: data.entity.name,
            schoolNo: data.entity.schoolNo,
            idCard: data.entity.idCard,
            "schoolClass": data.entity.schoolClass.id
          };
        }
      });
    },
